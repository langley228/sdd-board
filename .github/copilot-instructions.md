...
- 本專案名稱為 `sdd-board`。
- `sdd-board` 是一個將 OpenSpec 從 CLI 延伸到 Web 的多專案協作平台。
- 目前收斂出的核心流程為：
...
- 即時互動：Socket.io
- AI 整合：Vercel AI SDK
- Web 編輯器：Monaco（但需經過 editor abstraction 封裝）
- 認證方案：Auth.js
- 測試方案：Jest + Supertest + Playwright
- 部署原則：可容器化

以上內容代表目前第一候選，不代表不可更動的最終定案。
...
- 模組切分應保持清楚：`project`、`room`、`ticket`、`repository`、`ai`、`automation`、`openspec`。
...
- 當產品方向、流程、架構原則有變更時，同步更新 `README.md` 與 `docs/`。
- 當程式規範有變更時，同步更新 `docs/coding-standards.md`。
- 若技術候選有重大調整，同步更新 `docs/tech-options.md`。
