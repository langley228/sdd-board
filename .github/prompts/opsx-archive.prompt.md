...
**輸入**：可在 `/opsx:archive` 後選擇性指定變更名稱（例如：`/opsx:archive add-auth`）。若省略，確認是否能從對話上下文推斷。若模糊不清，**必須**提示可用的變更。
...
**警告：**
- 已封存，但有 2 個未完成的 artifacts
- 已封存，但有 3 個未完成的任務
- Delta spec 同步已跳過（使用者選擇跳過）
...
**變更：** <change-name>
**目標：** 由 `planningHome.changesDir`/YYYY-MM-DD-<name>/ 導出的封存路徑
...
- 清楚顯示發生了什麼
- 若要求同步，使用 Skill 工具呼叫 `openspec-sync-specs`（agent 驅動）
